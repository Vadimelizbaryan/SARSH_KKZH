...
Этот проект уже умеет принимать фото бланков через Telegram-бота и сохранять данные в систему через новую Supabase Edge Function `sharsh-telegram`.
...
```text
https://api.telegram.org/bot<TELEGRAM_BOT_TOKEN>/setWebhook?url=https://ywecvlapdlaojpvijaqy.supabase.co/functions/v1/sharsh-telegram
```
...
```powershell
$body = @{
  url = "https://ywecvlapdlaojpvijaqy.supabase.co/functions/v1/sharsh-telegram"
