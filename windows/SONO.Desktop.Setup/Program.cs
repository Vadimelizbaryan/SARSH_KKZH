...
{
    private const string ReleaseBaseUrl = "https://vadimelizbaryan.github.io/SARSH_KKZH/windows/releases/SONO.Desktop/";
    private const string ManifestFileName = "package-manifest.json";
    private const string InstallFolderName = "SONO Desktop";
    private const string PreferredInstallBasePath = @"D:\SHARSH AI Apps";
    private const string DesktopShortcutName = "SONO Desktop.lnk";
    private const string InstalledExecutableName = "Sono.exe";
    private const string InstalledProcessName = "Sono";
    ...
    {
        _options = options;
        _logPath = Path.Combine(Path.GetTempPath(), "Sono.Setup.log");

        Text = options.SilentMode ? "SONO Update" : "SONO Setup";
        ...
        var titleLabel = new Label
        {
            ...
            Text = _options.SilentMode ? "Обновление SONO Desktop" : "Установка SONO Desktop"
        };
    ...
    private async Task RunInstallAsync()
    {
        ...
        try
        {
            ...
            SetStatus("Останавливаю запущенные копии SONO Desktop...", 4);
            ...
            {
                throw new FileNotFoundException("После установки не найден файл Sono.exe.", executablePath);
            }

            SetStatus("Запускаю SONO Desktop...", 98);
            StartInstalledApplication(executablePath, installDir, _options.RestartInBackground);

            SetStatus("Готово. SONO Desktop запущен.", 100);
            Log("Installer finished successfully.");

            if (!_options.SilentMode)
            {
                MessageBox.Show(
                    $"SONO Desktop установлен в:\n{installDir}\n\nПриложение уже запущено.",
                    "SONO Setup",
        ...
        catch (Exception error)
        {
            ...
            {
                MessageBox.Show(
                    $"Не удалось установить SONO Desktop.\n\n{error.Message}",
                    "SONO Setup",
                    MessageBoxButtons.OK,
    ...
    private async Task<ManifestSource> ResolveManifestSourceAsync()
    {
        ...
        {
            var localManifestPath = Path.Combine(AppContext.BaseDirectory, "SONO.Desktop", ManifestFileName);
    ...
    )
    {
        ...
        for (var index = 0; index < normalizedEntries.Count; index++)
        {
            ...
            SetStatus($"ÐžÐ±Ð½Ð¾Ð²Ð»ÑÑŽ {index + 1}/{total}: {entry.Path}", percent);
    ...
    private static string GetSafeDestinationPath(string installDir, string manifestPath)
    {
        ...
        {
            throw new InvalidOperationException($"ÐžÐ±Ð½Ð°Ñ€ÑƒÐ¶ÐµÐ½ Ð½ÐµÐ±ÐµÐ·Ð¾Ð¿Ð°ÑÐ½Ñ‹Ð¹ Ð¿ÑƒÑ‚ÑŒ Ð² Ð¿Ð°ÐºÐµÑ‚Ðµ: {manifestPath}");
        }
